...
(async () => {
  ...
  await page.goto(process.env.REPETITA_URL || 'http://127.0.0.1:4173/', { waitUntil: 'networkidle' });
  ...
  const layout = await page.evaluate(() => ({
    ...
    report: document.querySelector('#resultContent').textContent.includes('VERIFIED BY REPETITA GRAVITY'),
  ...
  if (!layout.report || !layout.semanticGate) throw new Error(`Certified report missing: ${JSON.stringify(layout)}`);
